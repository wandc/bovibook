...
/**
  File to read addican sherbrooke lab files from DB and into something we can use. 
 */
...
class forageAnalysisReportsParseAgriAnalyze {
    ...
    function main() {
        ...
        $sql = "SELECT id,data::bytea,file_name FROM documents.forage_analysis_agri_analyze ORDER BY file_name DESC";
...
if ((defined('STDIN')) AND ( !empty($argc) && strstr($argv[0], basename(__FILE__)))) {
    $cls = new forageAnalysisReportsParseAgriAnalyze();
    $cls->main();
} else {
    //nothing
}
